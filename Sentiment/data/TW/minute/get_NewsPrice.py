...
def get_news_udn(NOW, KEYWORD ="大盤", start = None):
    ...
    for link, time in tqdm(links_time_all.items()):
        """
...
if __name__ == '__main__':
    ...
    '''
    NOW = False
    TICKERS = sys.argv[1] if len(sys.argv) > 2 else '0050.TW'
    KEYWORD = sys.argv[2] if len(sys.argv) > 3 else '大盤'
    INTERVAL = sys.argv[3] if len(sys.argv) > 4 else '1m'
    MEDIA = sys.argv[4] if len(sys.argv) > 5 else 'UDN'
    
    # If get 'now' news than don't need start time
    start = '2024-01-01 00:00:00' if not NOW else None
